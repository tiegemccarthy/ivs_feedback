...
def main(exp_code, db_name):
    ...
    for i in range(0, len(performance)):
         if performance[i] == None:
            ...
            sql_station = "INSERT IGNORE INTO {} (ExpID, Performance, Date, Pos_X, Pos_Y, Pos_Z, Pos_U, Pos_E, Pos_N, W.RMS_del, Problem, Problem_String, Analyser) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s);".format(station_id[i])
            data = [meta[0], performance[i], meta[2], position[i][0], position[i][1], position[i][2], position[i][3], position[i][4], position[i][5], delays[i], problems[0][i], problems[1][i], meta[1]]
